...
import { Cards } from '../../SortedCardRelationships/views/Cards'
import { RelationshipSelect } from './FilteredRelationshipSelect'

export type FilteredFieldProps = FieldProps<typeof controller> & {
...
function LinkToRelatedItems({
  ...
  itemId: string | null
  value: FilteredFieldProps['value'] & { kind: 'many' | 'one' }
  list: ListMeta
  refFieldKey?: string
}) {
...
export const FilteredRelationshipField = ({
  field,
  itemValue,
  autoFocus,
  value,
    ...
    serialize: (state) => {
      if (state.kind === 'many') {
        ...
        const disconnect = [...state.initialIds]
          ...
          .map((id) => ({ id }))
        const connect = [...state.currentIds].filter((id) => !state.initialIds.has(id)).map((id) => ({ id }))
